import { act, LegacyRef, memo, useMemo, useReducer, useRef, useState } from "react";
...
const reducer = (state: JobState, action: JobAction): JobState => {
    switch (action.type) {
        ...
        case DELETE_JOB: return {
            ...state,
            jobs: state.jobs.filter((_: string, index: number) => index !== action.payload)
        }
...
const TestComponent: React.FC = () => {
    ...
    return (
        <div style={{ textAlign: "center", marginTop: "20px" }}>
            ...
            <br />
            <ul>
                {jobs.map((job: string, index: number) => <li
                    key={index}
